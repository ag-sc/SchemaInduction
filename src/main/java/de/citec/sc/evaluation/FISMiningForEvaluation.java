 ...
 * @author sherzod
 */
public class FISMiningForEvaluation {

    public static void main(String[] args) {
        try {
            induceProperties("transactionTables/indexobject.txt", "goldStandardObjectProperties.txt", "object");
        } catch (IOException ex) {
            Logger.getLogger(FISMiningForEvaluation.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public static void induceProperties(String indexPath, String goldStandard, String type) throws IOException {
        ...
        DocumentUtils.writeListToFile(f1.getPath(), header, true);
        DocumentUtils.writeListToFile(f2.getPath(), header, true);
        DocumentUtils.writeListToFile(f3.getPath(), header2, true);
            ...
            try {
                ...
                for (double t = 1.0; t >= 0.1; t = t - 0.1) {
                    ...
                    try {
                        ...
                        processDomainItemsets(property, domainPatterns, goldDomain, f2.getPath(), round(t, 2));
                        processRangeItemsets(property, rangePatterns, goldRange, f1.getPath(), round(t, 2));
                        processDomainRangeItemsets(property, domainRangePatterns, goldDomain, goldRange, f3.getPath(), round(t, 2));
                    } catch (Exception e2) {
